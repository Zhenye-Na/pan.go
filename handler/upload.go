...
func UploadHandler(w http.ResponseWriter, r *http.Request) {
    if r.Method == "GET" {
        ...
        // 另一种返回方式:
        // 动态文件使用 http.HandleFunc 设置, 静态文件使用到 http.FileServer 设置 (见 main.go)
        // 所以直接 redirect 到 http.FileServer 所配置的 url
        ...
        if err != nil {
            fmt.Printf("Failed to upload file, err:%s\n", err.Error())
            return
        }
        defer newFile.Close()
...
func DownloadHandler(w http.ResponseWriter, r *http.Request) {
    ...
    // attachment 表示文件将会提示下载到本地，而不是直接在浏览器中打开
